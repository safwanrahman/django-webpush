...
window.addEventListener('load', function () {
  ...
  if (this.localStorage.getItem('push_subscribed')) {
    isPushEnabled = true;
  ...
  subBtn.addEventListener('click',
    async function () {
      if (isPushEnabled) {
        registration = await initialiseServiceWorker();
...
async function initialiseServiceWorker() {
  ...
  if ('serviceWorker' in navigator) {
    ...
    var register = await navigator.serviceWorker.register(serviceWorker)
      .then(
        function (reg) {
          initialiseState(reg);
          return reg;
